...
def process_video(video_name, labels_dict):
    ...
    if os.path.exists(out_path):
        print(f"  [略過] 視覺化影片已存在，不重複產生：labeled_{video_name}.mp4")
        return "skipped"
...
def main():
    ...
    for vname in all_videos:
        res = process_video(vname, labels_dict)
        if res is True:
            success_count += 1
        elif res == "skipped":
            skipped_count += 1
    print(f"\n批次處理結束！新產生 {success_count} 部影片，略過 {skipped_count} 部既有影片。")
    print(f"影片存放在：{OUTPUT_VIDEO_DIR}")
